...
class PDFOCRApp(QMainWindow):
  ...
  def initUI(self):
    ...
    self.save_data_btn.clicked.connect(self.save_data_with_dialog)
    control_layout.addWidget(self.save_data_btn)

    self.load_data_btn = QPushButton("Load Data")
    self.load_data_btn.clicked.connect(self.load_data_with_dialog)
  ...
  def update_document_info(self):
    ...
    if self.current_document_idx >= 0:
      ...
      self.page_label.setText(f"Page: {doc.current_page + 1}/{doc.get_page_count()}")
    else:
  ...
  def on_image_click(self, event):
    ...
    if pixmap:
      ...
      scale_x = (
        pixmap_size.width() / label_size.width() if label_size.width() > 0 else 1
      )
      scale_y = (
        pixmap_size.height() / label_size.height() if label_size.height() > 0 else 1
      )
  ...
  def combine_selected_boxes(self):
    ...
    doc = self.documents[self.current_document_idx]
    new_text_boxes = [
      box for idx, box in enumerate(self.text_boxes) if idx not in self.selected_indices
    ]
    ...
    if ok and label:
      if label not in self.available_labels:
        ...
        if len(self.selected_indices) == 1:
          self.label_selector.setCurrentIndex(self.available_labels.index(label))
      else:
  ...
  def show_model_training(self):
    ...
    if not has_labels:
      self.status_label.setText(
        "No labeled text boxes found. Please label some boxes first."
      )
      QMessageBox.warning(
        self,
        "No Labels",
  ...
  def show_model_prediction(self):
    ...
    if doc.current_page >= doc.get_page_count():
      ...
      QMessageBox.warning(self, "Invalid Page", "The current page number is invalid.")
      return
  ...
  def apply_predictions(self, predictions):
    ...
    width, height = (
      image.size if hasattr(image, "size") else (image.width, image.height)
    )
    ...
    self.status_label.setText(
      f"Applied {len(doc.page_boxes[doc.current_page])} labeled boxes from model predictions"
    )
